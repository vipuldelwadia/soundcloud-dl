...
function settags() {
    ...
    if $curlinstalled; then
        curl -s -L --user-agent 'Mozilla/5.0' "$imageurl" -o '/tmp/tempscdl1.jpg'
    else
        wget --max-redirect=1000 --trust-server-names -U 'Mozilla/5.0' -O '/tmp/tempscdl1.jpg' "$imageurl"
    fi
    if [ "$writags" = "1" ] ; then
        eyeD3 --add-image=/tmp/tempscdl1.jpg:ILLUSTRATION -a "$artist" -Y $(date +%Y) -G "$genre" -t "$title" -A "$album" "$filename" &>/dev/null
...
function downallsongs() {
    ...
    do
        ...
        if [ "$playlist" = "playlist" ] ; then 
            ...
            genre=$(echo "$songs" | sed -n "$i"p | tr ">" "\n" | grep "</genre" | cut -d "<" -f 1)
            imageurl=$(echo "$songs" | sed -n "$i"p | tr ">" "\n" | grep "</artwork-url" | cut -d "<" -f 1 | sed 's/large/original/g')
...
function downset() {
    ...
    do
        ...
        imageurl=$(echo "$songpage" | tr ">" "\n" | grep -A1 '<div class="artwork-download-link"' | cut -d '"' -f 4 | tr " " "\n" | grep 'http')
        genre=$(echo "$songpage" | tr ">" "\n" | grep -A1 '<span class="genre search-deprecation-notification" data="/tags/' | tr ' ' "\n" | grep '</span' | cut -d "<" -f 1)
        album=$(echo "$page" | sed s/'<meta content='/\n/g | grep 'property="og:title"' | cut -d '=' -f 4 | cut -d '"' -f 4)
        #------------------------#
        # DL
        if $curlinstalled; then
            songurl=$(curl -s -L --user-agent 'Mozilla/5.0' "https://api.sndcdn.com/i1/tracks/$id/streams?client_id=$clientID" | cut -d '"' -f 4 | sed 's/\\u0026/\&/g')
